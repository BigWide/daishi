...
def parseData(rec):
	...
	if rec.uID==0x506:  #GpsAntAngles
		...
		Hour = int(pSum[0,8],2)        
		daishi_info.DateTime = "%s%s-%s-%s %s:%s:%s" % (Century,Year,Month,Day,Hour+8,Minute,float(Second)+HSecond)
...
def daishi():
	...
	while not rospy.is_shutdown():
		reclen = clib.CAN_ChannelReceive(dwDeviceHandle, 1 , byref(rec1), 22, 20)
		reccount = reccount + reclen
		parseData(rec1)
...
if __name__ == '__main__':
	...
	clib = cdll.LoadLibrary('./libCanCmd.so')
